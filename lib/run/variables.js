...
*/
    ...
    setMethod(p, m, val, d = 0) {
        
        if (this.val[p[d]]) {
            
            this.val[p[d]].setMethod(p, m, val, ++d);
        }
        else {

            if (d < p.length) throw new Maus(p.slice(0, d - 1).join("'Z ") + " NO HAS " + p[d], 404);
            this.fncs[m] = val;
    ...
    getMethod(p, m) {
        ...
        if (!(m in obj.fncs)) throw new Maus(p.join("'Z ") + " NO HAS " + m, 404);
        return obj.fncs[m];
