...
function interpretProgram() {
    ...
    if (global.idfIdx < tokens.length) {
        ...
        switch (tokens[global.idfIdx]) {
            ...
            case "HAS A":
                ...
                idf.push(name);
                global.vars[0].setSlot(idf,val,true);
                return;
            ...
            case "R":
                ...
                //var name = identify(tokens.slice(++t));
                //console.log(idf);
                global.vars[0].setSlot(idf, evalExpr());
                return;

            case "IS NOW A":
                t = global.idfIdx;
               // console.log(global.vars)
                var to = cast(global.vars[0].getSlot(idf), tokens[++t]);
                //var name = identify(tokens.slice(++t));
                //console.log(to);
                t = 2;
                global.vars[0].setSlot(idf, to);
                return;
    ...
    t = -1;
    global.vars[0].val.IT = evalExpr();
    //console.log(vars[0].val.IT, t)
...
/*function identify(_tokens_) {
    ...
    do {
        ...
        idf.push(_tokens_[i] ? _tokens_[i].replace(/'Z$/, "") : "LOOL");
        idfIdx++;


    } while (_tokens_[i] && _tokens_[i++].endsWith("'Z"))
