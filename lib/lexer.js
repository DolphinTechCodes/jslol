...
var i, tokens = [], inMLcomment = false, textlines = [];;
    ...
    for (i; i < textlines.length; i++) {
        ...
        while (c < textline.length) {
            ...
            if (/\s/.test(textline[c])) {
                ...
                inWord = true;
                var inWord_ = true;
                while ((inWord && inWord_) || keywords.possible(token)) {

                    // console.log(token, keywords.possible(token), inWord, inWord_)
                    inWord_ = inWord;
                ...
                if (!inWord_) { token = token.slice(0, -1); c--; }
